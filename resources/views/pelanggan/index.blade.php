<x-app-layout>
    <div x-data="{ open: false, openEdit: false, selectedPelanggan: null }" x-cloak>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-[#6c0ba9] leading-tight">
                {{ __('Halaman Pelanggan') }}
            </h2>
                ...
                <tbody class="divide-y divide-purple-200">
                    @foreach ($pelanggans as $pelanggan)
                        <tr class="group hover:bg-[#6c0ba9] hover:text-white text-dark transition">
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input id="table-checkbox-{{ $pelanggan->id }}" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-white focus:ring-2 focus:ring-blue-500">
                                    <label for="table-checkbox-{{ $pelanggan->id }}" class="sr-only">
                                        Table checkbox
                                    </label>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if($pelanggan->status_pelanggan === 'aktif')
                                    <span class="inline-flex items-center justify-center w-20 px-2 py-1 text-xs font-semibold text-white bg-green-500 rounded">
                                        Aktif
                                    </span>
                                @elseif($pelanggan->status_pelanggan === 'suspend')
                                    <span class="inline-flex items-center justify-center w-20 px-2 py-1 text-xs font-semibold text-white bg-yellow-600 rounded">
                                        Suspend
                                    </span>
                                @else
                                    <span class="inline-flex items-center justify-center w-20 px-2 py-1 text-xs font-semibold text-white bg-red-500 rounded">
                                        Putus
                                    </span>
                                @endif
                            </td>
        ...
        <div class="mt-4">
            {{ $pelanggans->links('components.pagination') }}
        </div>
